...
DEFAULT_CREDENTIALS = {
    "users": {
        "admin": {
            "password": "1234",  # MD5 hash of "password"
            "is_admin": True,
            "manager_password": "clinton"  # MD5 hash of "manager123"
        },
        "cashier": {
            "password": "1234",  # MD5 hash of "password"
            "is_admin": False,
...
def save_user_credentials(username, password, is_admin=False, manager_password=None):
    ...
    if manager_password:
        config["users"][username]["manager_password"] = hash_password(manager_password)
...
class HotelApp:
    ...
    def setup_sales_db(self):
        # Each user will have their own sales database
    ...
    def setup_meals_db(self):
        # Meals database is shared among all users
    ...
    def record_sale(self, meal, quantity, amount):
        ...
        try:
            ...
            self.sales_cursor.execute("INSERT INTO sales (user, date, meal, quantity, amount) VALUES (?, ?, ?, ?, ?)",
                                      (self.current_user, date, meal, quantity, amount))
            self.sales_conn.commit()
        except Exception as e:
    ...
    def show_daily_sales(self):
        date = datetime.now().strftime('%Y-%m-%d')
        self.sales_cursor.execute('''SELECT meal, SUM(quantity), SUM(amount) 
                                    FROM sales 
                                    WHERE date=? AND user=?
                                    GROUP BY meal''',
                                  (date, self.current_user))
        rows = self.sales_cursor.fetchall()

        self.sales_cursor.execute('''SELECT SUM(amount) 
                                     FROM sales 
                                     WHERE date=? AND user=?''',
                                  (date, self.current_user))
    ...
    def clear_today_sales(self, sales_window=None):
        def do_clear():
            date = datetime.now().strftime('%Y-%m-%d')
            self.sales_cursor.execute('DELETE FROM sales WHERE date=? AND user=?', (date, self.current_user))
            self.sales_conn.commit()
        ...
        def check_pw():
            user_config = self.config["users"].get(self.current_user, {})
            stored_hash = user_config.get("manager_password", "")

            if hash_password(pw_var.get()) == stored_hash:
                pw_window.destroy()
                do_clear()
            else:
    ...
    def show_login_page(self):
        ...
        # Change credentials button - now with background color
        if self.config["users"]:  # Only show if there are existing users
    ...
    def show_add_user_dialog(self):
        ...
        add_user_win.geometry("500x500")  # Increased height to accommodate manager password field
        add_user_win.configure(bg=BG_COLOR)
        add_user_win.transient(self.root)
        add_user_win.grab_set()

        # User selection (for admin to choose which user to add)
        tk.Label(add_user_win, text="Select Admin User:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=5)

        admin_user_var = tk.StringVar()
        admin_users = [u for u, data in self.config["users"].items() if data.get("is_admin", False)]
        admin_user_dropdown = ttk.Combobox(add_user_win, textvariable=admin_user_var,
                                           values=admin_users, font=FONT_MEDIUM)
        admin_user_dropdown.pack(pady=5, ipady=3)

        # Admin password verification
        tk.Label(add_user_win, text="Admin Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=5)
        admin_pw_var = tk.StringVar()
        tk.Entry(add_user_win, textvariable=admin_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        # New user details
        tk.Label(add_user_win, text="New Username:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=5)
        new_user_var = tk.StringVar()
        tk.Entry(add_user_win, textvariable=new_user_var,
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        tk.Label(add_user_win, text="New Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=5)
        new_pw_var = tk.StringVar()
        tk.Entry(add_user_win, textvariable=new_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        tk.Label(add_user_win, text="Confirm Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=5)
        confirm_pw_var = tk.StringVar()
        tk.Entry(add_user_win, textvariable=confirm_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        # Admin checkbox
        is_admin_var = tk.BooleanVar()
        tk.Checkbutton(add_user_win, text="Admin User", variable=is_admin_var,
                       font=FONT_MEDIUM, bg=BG_COLOR, fg=FG_COLOR,
                       selectcolor=BG_COLOR, activebackground=BG_COLOR).pack(pady=5)

        # Manager password (only shown when admin user is selected)
        manager_pw_frame = tk.Frame(add_user_win, bg=BG_COLOR)
        manager_pw_frame.pack(pady=5)

        def update_manager_pw_visibility(*args):
            if is_admin_var.get():
                manager_pw_frame.pack(pady=5)
            else:
                manager_pw_frame.pack_forget()

        is_admin_var.trace('w', update_manager_pw_visibility)

        tk.Label(manager_pw_frame, text="Manager Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(side=tk.LEFT)
        manager_pw_var = tk.StringVar()
        tk.Entry(manager_pw_frame, textvariable=manager_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR, width=15).pack(side=tk.LEFT, padx=5)

        def add_user():
            admin_user = admin_user_var.get()
            ...
            manager_pw = manager_pw_var.get() if is_admin else None

            # Verify admin user and password
            admin_data = self.config["users"].get(admin_user)
            if not admin_data or not admin_data.get("is_admin", False):
                messagebox.showerror("Error", "Selected user is not an admin", parent=add_user_win)
                return

            if admin_data["password"] != hash_password(admin_pw):
                messagebox.showerror("Error", "Invalid admin password", parent=add_user_win)
                return

            # Validate inputs
            if not new_user or not new_pw:
                messagebox.showerror("Error", "Username and password are required", parent=add_user_win)
                return

            if new_pw != confirm_pw:
                messagebox.showerror("Error", "Passwords do not match", parent=add_user_win)
                return

            if is_admin and not manager_pw:
                messagebox.showerror("Error", "Manager password is required for admin users", parent=add_user_win)
                return

            if new_user in self.config["users"]:
                messagebox.showerror("Error", "Username already exists", parent=add_user_win)
                return
        ...
        btn_frame = tk.Frame(add_user_win, bg=BG_COLOR)
        ...
        tk.Button(btn_frame, text="Cancel", command=add_user_win.destroy,
                  bg=ERROR_COLOR, fg=FG_COLOR, font=FONT_MEDIUM,
                  padx=15, pady=5).pack(side=tk.LEFT, padx=10)
    ...
    def show_change_credentials(self):
        ...
        change_window.geometry("500x450")
        change_window.configure(bg=BG_COLOR)
        change_window.transient(self.root)
        change_window.grab_set()

        # User selection
        tk.Label(change_window, text="Select User:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=10)

        user_var = tk.StringVar()
        user_dropdown = ttk.Combobox(change_window, textvariable=user_var,
                                     values=list(self.config["users"].keys()), font=FONT_MEDIUM)
        user_dropdown.pack(pady=5, ipady=3)

        # Current password
        tk.Label(change_window, text="Current Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=10)
        current_pw_var = tk.StringVar()
        tk.Entry(change_window, textvariable=current_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        # New password
        tk.Label(change_window, text="New Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=10)
        new_pw_var = tk.StringVar()
        tk.Entry(change_window, textvariable=new_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        # Confirm password
        tk.Label(change_window, text="Confirm Password:", font=FONT_MEDIUM,
                 bg=BG_COLOR, fg=FG_COLOR).pack(pady=10)
        confirm_pw_var = tk.StringVar()
        tk.Entry(change_window, textvariable=confirm_pw_var, show="*",
                 font=FONT_MEDIUM, bg="#2a2a40", fg=FG_COLOR).pack(pady=5, ipady=3)

        # Manager password (only for admin users)
        manager_pw_frame = tk.Frame(change_window, bg=BG_COLOR)
        manager_pw_frame.pack(pady=10)

        def update_manager_pw_visibility(*args):
            selected_user = user_var.get()
            if selected_user in self.config["users"] and self.config["users"][selected_user].get("is_admin", False):
                manager_pw_frame.pack(pady=10)
            else:
                manager_pw_frame.pack_forget()

        user_var.trace('w', update_manager_pw_visibility)
        ...
        def save_changes():
            ...
            if not username or not current_pw:
                messagebox.showerror("Error", "Please select a user and enter current password")
                return
            ...
            if user_data["password"] != hash_password(current_pw):
        ...
        btn_frame = tk.Frame(change_window, bg=BG_COLOR)
        btn_frame.pack(pady=20)
    ...
    def show_main_system(self):
        ...
        # Title marquee
        title_marquee = Marquee(self.root, text=f"HOTEL MANAGEMENT SYSTEM - {self.current_user}")
    ...
    def calculate_total(self):
        ...
        self.bill_txt.insert(tk.END, f"Attendant: {self.current_user}\n")
