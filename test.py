...
class HotelApp:
    def __init__(self, root):
        ...
        # Initialize databases in the main thread
    ...
    def setup_meals_db(self):
        ...
        self.meals_cursor.execute('''
            CREATE TABLE IF NOT EXISTS meals (
                ...
                name TEXT,
                price REAL,
                UNIQUE(category, name)  -- Ensure no duplicate meals in same category
            )
        ''')
    ...
    def show_daily_sales(self):
        date = datetime.now().strftime('%Y-%m-%d')
        self.sales_cursor.execute('SELECT meal, SUM(quantity), SUM(amount) FROM sales WHERE date=? GROUP BY meal', (date,))
    ...
    def show_main_system(self):
        ...
        for idx, (category, items) in enumerate(self.menu_items.items()):
            row = (idx // 2) + 1
            col = idx % 2
            
            # Create frame for category
            frame = tk.LabelFrame(main_frame, text=category, fg=ACCENT_COLOR, bg="#1e1e2e",
                                font=FONT_MEDIUM, bd=2, relief=tk.GROOVE)
            frame.grid(row=row, column=col, sticky="nsew", padx=5, pady=5)
            frame.grid_propagate(False)
            self.meal_frames[category] = frame
            ...
            for i, (item, price) in enumerate(items.items()):
                ...
                tk.Label(item_frame, text=f"{item} (ksh{price}):", 
    ...
    def remove_existing_meal_dialog(self):
        ...
        def delete_meal():
            ...
            if messagebox.askyesno("Confirm", f"Are you sure you want to remove {name} from {category}?", parent=remove_meal_win):
    ...
    def payment_method_dialog(self):
        ...
        tk.Label(details_frame, text="Payment Details:", 
                font=FONT_MEDIUM, bg=BG_COLOR, fg=FG_COLOR).pack()
        
        details_var = tk.StringVar(value=self.payment_method_used["details"])
        tk.Entry(details_frame, textvariable=details_var, font=FONT_MEDIUM, 
                bg="#2a2a40", fg=FG_COLOR, width=30).pack()
        ...
        def proceed():
            ...
            self.payment_method_used = {"method": method, "details": details}
            dialog.destroy()
        ...
        def mpesa_payment():
            ...
            def send_mpesa():
                ...
                if not phone or not phone.isdigit() or len(phone) < 10:
                    messagebox.showerror("Error", "Enter a valid client phone number.", parent=mpesa_win)
                    return
                ...
                try:
                    ...
                    messagebox.showerror("Error", "Enter a valid amount.", parent=mpesa_win)
                    return
                
                mpesa_win.destroy()
                
                # In a real implementation, you would call the Mpesa API here
                messagebox.showinfo("Mpesa Payment", 
                                  f"A payment request has been sent to {phone}.\n"
                                  f"The client will receive a prompt to enter their Mpesa PIN to transfer Ksh {amt:.2f}.",
                                  parent=self.root)
                
                self.payment_method_used = {"method": "Mpesa", 
                                          "details": f"Phone: {phone}, Amount: {amt:.2f}"}
            
            btn_frame = tk.Frame(mpesa_win, bg=BG_COLOR)
            btn_frame.pack(pady=10)
            
            tk.Button(btn_frame, text="OK", font=FONT_MEDIUM, 
